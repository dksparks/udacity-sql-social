CREATE TABLE users (
  ...
  CONSTRAINT username_nonempty CHECK (
...
CREATE TABLE topics (
  ...
  CONSTRAINT topic_name_nonempty CHECK (
    LENGTH(TRIM(name)) > 0
  )
);
...
CREATE TABLE posts (
  ...
  url VARCHAR(2000),
  content TEXT,
  CONSTRAINT title_nonempty CHECK (
    LENGTH(TRIM(title)) > 0
  ),
...
CREATE TABLE comments (
  ...
  parent_comment_id BIGINT REFERENCES comments (id)
    ON DELETE CASCADE,
  -- comments must have a parent_post_id or
  --   a parent_comment_id, but not both
  ...
  CONSTRAINT comment_content_nonempty CHECK (
